...
A lightweight, robust web search and extraction extension for the Pi coding agent. It consolidates multiple providers (Exa, Firecrawl, Tavily, Browserbase) behind a clean set of tools with built-in fallback routing, cooldown tracking, and zero bulky SDK dependencies.
...
By default, the extension requires **no configuration** and will automatically use environment variables (`EXA_API_KEY`, `FIRECRAWL_API_KEY`, `TAVILY_API_KEY`, `BROWSERBASE_API_KEY`) or default to the free Exa MCP endpoint.
...
```json
{
  "providers": {
    ...
    "browserbase": { "apiKey": "BROWSERBASE_API_KEY" }
  },
  "tools": {
    "web_search": {
      "providers": ["exa", "firecrawl", "tavily", "browserbase", "exa-free"],
      "maxResults": 8,
      "timeoutSeconds": 30
    },
    "code_search": {
    ...
    "web_extract": {
      "providers": ["firecrawl", "tavily", "browserbase"]
    }
  }
}
```
