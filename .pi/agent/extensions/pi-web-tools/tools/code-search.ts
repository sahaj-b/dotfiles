...
import type { WebToolsConfig, CodeSearchDetails } from "../types.ts";
...
export function createCodeSearchTool(config: WebToolsConfig, providers: Provider[]) {
	...
	return {
		name: "code_search",
		label: "Code Search",
		description: "Search code-specific sources (GitHub, Stack Overflow, docs). Errors directly on failure — use web_search as an alternative if this fails.",
		...
		async execute(
			...
			onUpdate?.({
				content: [textContent(`Searching code for ${JSON.stringify(query)}...`)],
			...
			for (const provider of orderedProviders) {
				...
				try {
					...
					const details: CodeSearchDetails = { query, maxTokens, provider: provider.id };
