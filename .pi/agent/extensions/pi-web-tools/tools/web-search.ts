...
import type { WebSearchDetails, WebToolsConfig } from "../types.ts";
	...
	return {
		...
		parameters: Type.Object({
			...
			maxResults: Type.Optional(
				Type.Number({ description: "Maximum results to return (1-20)." }),
			),
			provider: Type.Optional(
				Type.String({
					description: "Force a specific provider (skip fallback chain).",
				}),
			),
		}),
		...
		async execute(
			...
			const maxResults = Math.max(
				1,
				Math.min(20, params.maxResults ?? toolConfig.maxResults ?? 8),
			);
			...
			const { result, provider, fallbacksUsed } = await executeWithFallback(
				...
				"search",
				async (p) => {
					if (!isSearchCapable(p))
						throw new Error(`${p.id} doesn't support search`);
		...
		renderCall(
			args: { query: string; maxResults?: number; provider?: string },
			theme: any,
			ctx: any,
		) {
		...
		renderResult(
			...
			if (result.isError)
				return connectorText(
					...
					theme.fg(
						"error",
						`✗ ${result.content?.[0]?.text || "Search failed"}`,
					),
				);
			...
			if (d?.fallbacksUsed?.length)
				text += theme.fg("dim", ` [fallbacks: ${d.fallbacksUsed.join(", ")}]`);
