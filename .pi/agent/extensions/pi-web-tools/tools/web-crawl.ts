...
import type { WebCrawlDetails, WebToolsConfig } from "../types.ts";
...
export function createWebCrawlTool(
	...
	return {
		...
		label: "󰖟 Crawl",
		description:
			"Crawl a website starting from a URL, discovering and extracting content from linked pages. Useful for documentation sites.",
		parameters: Type.Object({
			url: Type.String({ description: "Starting URL to crawl." }),
			maxDepth: Type.Optional(
				Type.Number({ description: "Max link depth (default 2)." }),
			),
			maxPages: Type.Optional(
				Type.Number({ description: "Max pages to crawl (default 10)." }),
			),
			instructions: Type.Optional(
				Type.String({
					description: "Natural language instructions to guide the crawler.",
				}),
			),
		}),
		...
		async execute(
			...
			if (orderedProviders.length === 0)
				throw new Error("No crawl providers configured");
			...
			const { result, provider } = await executeWithFallback(
				...
				async (p) => {
					if (!isCrawlCapable(p))
						throw new Error(`${p.id} doesn't support crawl`);
					return p.crawl(
						{ url, maxDepth, maxPages, instructions: params.instructions },
						signal,
					);
				},
				signal,
		...
		renderCall(
			args: { url: string; maxDepth?: number; maxPages?: number },
			theme: any,
			ctx: any,
		) {
			let text = `${animatedBullet(ctx, theme)} ${theme.fg("toolTitle", theme.bold(theme.fg("border", "󰖟 ") + "Crawl "))}`;
		...
		renderResult(
			...
			if (result.isError)
				return connectorText(
					ctx,
					theme,
					theme.fg("error", `✗ ${result.content?.[0]?.text || "Crawl failed"}`),
				);
