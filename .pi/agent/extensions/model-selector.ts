			...
			if (fs.existsSync(STATE_FILE)) {
				...
				return {
					...
					recentSelections:
						...
						typeof parsed.recentSelections === "object"
							? parsed.recentSelections
							: {},
				};
			}
...
class ModelSelectorComponent {
	...
	render(width: number): string[] {
		...
		const searchRender = this.searchInput.render(
			innerWidth - visibleWidth(searchLabel) - 2,
		);
		...
		if (!this.loaded) {
			lines.push(
				this.boxLine(this.theme.fg("muted", "  loading..."), innerWidth),
			);
	...
	private renderModelLine(
		...
		const imgSlot = item.model.input?.includes("image")
			? this.theme.fg("success", "󰋩 ")
			: "  ";
		...
		const gap = Math.max(
			1,
			innerWidth - visibleWidth(leftPart) - visibleWidth(rightPart),
		);
