...
function maybeBg(
	...
	if (token === DIFF_WORD_ADD_BG_TOKEN)
		return `${DIFF_WORD_ADD_BG_FALLBACK}${text}${WORD_DIFF_BG_RESET}`;
	if (token === DIFF_WORD_DEL_BG_TOKEN)
		return `${DIFF_WORD_DEL_BG_FALLBACK}${text}${WORD_DIFF_BG_RESET}`;
...
function compactStructuredDiffLines(
	...
	for (const range of ranges) {
		...
		if (hidden > 0 && !isFirstRange) compacted.push(hiddenDiffLine(hidden));
...
function colorDiffText(
	...
	const wordBgToken =
		line.type === "add" ? DIFF_WORD_ADD_BG_TOKEN : DIFF_WORD_DEL_BG_TOKEN;
...
export function renderStructuredDiff(
	...
	const width =
		explicitWidth !== undefined
			? Math.max(40, explicitWidth)
			: Math.max(40, terminalWidth(cwd) - Math.max(0, widthOffset));
