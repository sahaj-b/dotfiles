...
function maybeBg(theme: any, token: string, text: string, enabled: boolean): string {
	...
	if (token === DIFF_WORD_ADD_BG_TOKEN) return `${DIFF_WORD_ADD_BG_FALLBACK}${text}${WORD_DIFF_BG_RESET}`;
	if (token === DIFF_WORD_DEL_BG_TOKEN) return `${DIFF_WORD_DEL_BG_FALLBACK}${text}${WORD_DIFF_BG_RESET}`;
...
function styleAnsiVisibleRanges(
	...
	function emitChunk(chunk: string, highlighted: boolean): void {
		...
		out += maybeBg(theme, bgToken, chunk, useBackground);
...
function colorDiffText(line: StructuredDiffLine, text: string, theme: any, ranges: Array<[number, number]> = [], cwd?: string): string {
	...
	const wordBgToken = line.type === "add" ? DIFF_WORD_ADD_BG_TOKEN : DIFF_WORD_DEL_BG_TOKEN;
	return styleAnsiVisibleRanges(text, ranges, theme, fgToken, bgToken, wordBgToken, diffBackgroundEnabled(cwd));
}
...
export function renderStructuredDiff(diff: StructuredDiff, theme: any, expanded: boolean, cwd?: string, rowLimit?: number | null, path?: string, widthOffset = 0, explicitWidth?: number): string {
	if (diff.additions === 0 && diff.removals === 0) return theme.fg("muted", "no changes");
	const width = explicitWidth !== undefined ? Math.max(40, explicitWidth) : Math.max(40, terminalWidth(cwd) - Math.max(0, widthOffset));
...
export function attachDiffDetails(result: any, before: string | undefined, after: string | undefined, path?: string): any {
	...
	const extra = { toolDiff: diff, toolDiffWasNewFile: before === undefined };
 ...
 * pre-rendered text and breaks table borders), this regenerates the full
 * diff layout so table borders, column splits, and line numbers stay
 * correct at any width.
 */
export class DiffResult {
	private diff: StructuredDiff;
	private theme: any;
