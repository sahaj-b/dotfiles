...
function rebuildHighlightCache(rawPath: string, fileContent: string): HighlightCache | undefined {
...
function updateHighlightCacheIncremental(
	...
	if (cache.lang !== lang || cache.rawPath !== rawPath) return rebuildHighlightCache(rawPath, fileContent);
	if (!fileContent.startsWith(cache.rawContent)) return rebuildHighlightCache(rawPath, fileContent);
...
function getHighlightedLines(cache: HighlightCache | undefined, rawContent: string, rawPath: string): string[] {
	if (cache) return cache.highlightedLines;
	const lang = rawPath ? getLanguageFromPath(rawPath) : undefined;
	if (!lang || rawContent.length > 50_000) return normalizeDisplayText(rawContent).split("\n");
	try {
		const normalized = replaceTabs(normalizeDisplayText(rawContent));
		const highlighted = highlightCode(normalized, lang);
		return Array.isArray(highlighted) ? highlighted : normalizeDisplayText(String(highlighted)).split("\n");
	} catch {
...
function previewLines(
	...
	const lines = trimTrailingEmptyLines(getHighlightedLines(cache, fileContent, rawPath));
	const stem = treeConnector(theme, "│");
	return lines.slice(0, maxLines).map((line) => `${stem} ${line}`).join("\n");
}

function writePreviewLineLimit(context: any): number {
	if (context?.expanded) return context?.isPartial ? Number.POSITIVE_INFINITY : 200;
	return context?.isPartial ? writeLiveLines(context?.cwd) : writeCompletedLines(context?.cwd);
}

function formatWriteCall(args: any, theme: any, context: any, cache: HighlightCache | undefined): string {
	...
	if (contentText.length > 0) {
		text += `\n${previewLines(rawPath ?? "", contentText, cache, writePreviewLineLimit(context), theme)}`;
...
export function registerWrite(pi: ExtensionAPI, agent: any, cwd: string): void {
	...
	pi.registerTool({
		...
		renderCall(args: any, theme: any, context: any) {
			const component = context.lastComponent instanceof WriteRenderComponent
			...
			if (rawPath !== null && fileContent !== null) {
				component.cache = context.argsComplete
					...
					: updateHighlightCacheIncremental(component.cache, rawPath, fileContent);
			} else {
