...
function discoverModels(): ProviderModelConfig[] {
	...
	if (existsSync(CODEX_MODELS_CACHE)) {
		try {
			raw = JSON.parse(readFileSync(CODEX_MODELS_CACHE, "utf-8")).models || [];
		} catch {}
	}
	if (!raw.length) {
		try {
			raw =
				JSON.parse(
					execSync("codex debug models --bundled 2>/dev/null", {
						encoding: "utf-8",
						timeout: 10000,
					}),
				).models || [];
...
interface CodexUsageResponse {
	...
	additional_rate_limits?: Record<
		string,
		{ primary_window?: UsageWindow; secondary_window?: UsageWindow }
	>;
...
function formatDuration(seconds: number): string {
	...
	if (!parts.length) parts.push(`${remaining}s`);
...
function formatUsageInfo(data: CodexUsageResponse): string {
	...
	if (primary) {
		...
		if (primary.reset_at) {
			...
			lines.push(`  Resets in ${formatDuration(remaining)}`);
		}
	}
	...
	if (data.credits) {
		if (data.credits.unlimited) {
			...
			const bal =
				data.credits.balance != null ? Math.floor(data.credits.balance) : "?";
			const usd =
				data.credits.balance != null
					? (data.credits.balance * 0.04).toFixed(2)
					: "?";
	...
	if (data.rate_limit_reset_credits?.available_count) {
		lines.push(
			`\x1b[1mResets:\x1b[0m ${data.rate_limit_reset_credits.available_count} available`,
		);
	}
	...
	if (data.additional_rate_limits) {
		for (const [name, limit] of Object.entries(data.additional_rate_limits)) {
			if (limit.primary_window && limit.primary_window.used_percent > 0) {
				lines.push(
					`\x1b[1m${name}:\x1b[0m ${limit.primary_window.used_percent}% used`,
				);
			}
		}
	}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("codex-status", {
		...
		handler: async (_args, ctx) => {
			...
			if (!auth) {
				ctx.ui.notify(
					"Not authenticated. Run `codex-login` or `codex login`.",
					"warning",
				);
				return;
			}
			const expired = isTokenExpired(auth);
			const planType = parseJwtPayload(auth.tokens.access_token)?.[
				"https://api.openai.com/auth"
			]?.chatgpt_plan_type;
			ctx.ui.notify(
				expired
					? "Codex token expired. Run `codex-login` to refresh."
					: `Authenticated (Plan: ${planType || "unknown"})`,
			...
			if (expired) {
				...
				ctx.ui.notify(
					refreshed
						? "Token refreshed!"
						: "Token refresh failed. Run `codex-login`.",
	...
	pi.registerCommand("codex-ratelimit", {
		...
		handler: async (_args, ctx) => {
			...
			try {
				...
				ctx.ui.notify(
					`Failed to get rate limits: ${err.message || err}`,
					"error",
				);
			}
		},
