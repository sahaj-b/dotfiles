...
import type { SelectorState } from "./state.js";
...
export class ModelSelectorComponent {
  ...
  // Re-sort, re-filter, and snap selection back to the same model
  ...
  render(width: number): string[] {
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      if (!item) continue;
      const line = this.renderModelLine(item, i === this.selectedIndex, this.isCurrentModel(item.model), this.state.isFavorited(item.provider, item.id), innerWidth);
      lines.push(this.boxLine(line, innerWidth));
    }

    if (startIdx > 0 || endIdx < this.filteredModels.length) {
      lines.push(this.boxLine(this.theme.fg("muted", `  (${this.selectedIndex + 1}/${this.filteredModels.length})`), innerWidth));
    }

    const totalHidden = this.allModels.filter((m) => this.state.isProviderHidden(m.provider)).length;
    if (this.filteredModels.length > 0) {
      const sel = this.filteredModels[this.selectedIndex];
      const nameText = sel ? this.theme.fg("muted", `  ${sel.model.name}`) : "";
      if (totalHidden > 0) {
        const hiddenText = this.theme.fg("muted", ` ${this.state.hiddenProviders.join(", ")} (${totalHidden})`);
  ...
  private renderModelLine(item: ModelItem, selected: boolean, isCurrent: boolean, isFav: boolean, innerWidth: number): string {
    ...
    const ctxColor = item.model.contextWindow >= 500_000 ? "success" : item.model.contextWindow >= 300_000 ? "warning" : item.model.contextWindow >= 200_000 ? "text" : "muted";
