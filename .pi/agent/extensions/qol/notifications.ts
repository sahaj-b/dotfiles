...
export type QolNotificationKind =
	| "ready"
	| "direction"
	| "question"
	| "task-complete"
...
async function _playOnWindows(soundPath: string): Promise<void> {
	await _runSoundCmd("powershell", [
		"-c",
		`(New-Object Media.SoundPlayer '${soundPath.replace(/'/g, "''")}').PlaySync()`,
	]);
}

export async function playQolNotificationSound(
	soundKind: SoundKind,
	cwd?: string,
): Promise<void> {
...
export function piWindowFocused(sessionName?: string, cwd?: string): boolean {
	...
	const scriptPath = settingStringAllowEmpty(
		"notification.focusDetectionScript",
		DEFAULT_FOCUS_DETECTION_SCRIPT,
		cwd,
	);
	if (!scriptPath || !existsSync(scriptPath)) return false;
	const termTitle = settingStringAllowEmpty(
		"notification.termInitialTitle",
		DEFAULT_TERM_INITIAL_TITLE,
		cwd,
	);
...
export function notifyDesktop(title: string, body: string, cwd?: string): void {
	...
	try {
		if (os === "linux") {
			...
			spawn(
				"osascript",
				["-e", `display notification "${sb}" with title "${st}"`],
				{
					stdio: "ignore",
					detached: true,
				},
...
export function sanitizeNotificationPart(
	input: string,
	maxChars = DEFAULT_NOTIFICATION_BODY_MAX_CHARS,
): string {
	const cleaned = input
	...
	return cleaned.length > maxChars
		? `${cleaned.slice(0, Math.max(0, maxChars - 1))}…`
		: cleaned;
}

function notificationEnabledFor(
	...
	switch (kind) {
		case "ready":
			return settingBoolean("notification.onAgentReady", true, cwd);
		case "direction":
			return settingBoolean("notification.onDirectionNeeded", true, cwd);
		case "question":
			return settingBoolean("notification.onQuestion", true, cwd);
		case "task-complete":
...
export function sendQolNotification(
	...
	// Focus detection
	const suppressWhenFocused = settingBoolean(
		"notification.suppressWhenFocused",
		DEFAULT_SUPPRESS_WHEN_FOCUSED,
		cwd,
	);
	...
	const isFocused = suppressWhenFocused
		? piWindowFocused(sessionName, cwd)
		: false;

	// Play sound (unless suppressed by focus)
	const suppressSoundWhenFocused = settingBoolean(
		"notification.suppressSoundWhenFocused",
		false,
		cwd,
	);
	const soundKind =
		options?.sound !== undefined ? options.sound : qolKindToSoundKind(kind);
	...
	const title = sessionName
		? `π - ${sanitizeNotificationPart(sessionName, 40)}`
		: "π";
	const text = sanitizeNotificationPart(
		body,
		Math.max(
			40,
			Math.floor(
				settingNumber(
					"notification.bodyMaxChars",
					DEFAULT_NOTIFICATION_BODY_MAX_CHARS,
					cwd,
				),
			),
	...
	// Pi UI notification
	if (ctx?.hasUI && settingBoolean("notification.piUi", false, cwd))
