...
export interface QolContextUsageMessageDetails {
  usage: { contextWindow?: number; percent?: number; tokens: number };
  model: { contextWindow?: number; id: string; label: string; provider: string; thinking?: string };
  ...
  messageStats: { assistant: number; bash: number; branchEntries: number; compact: number; contextMessages: number; custom: number; toolResult: number; user: number };
