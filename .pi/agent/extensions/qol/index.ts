...
import { settingBoolean, settingString, settingNumber } from "./_config";
import { INSTALL_SYMBOL, CONTEXT_USAGE_MESSAGE_TYPE } from "./constants";
import { refreshGitState, renderStatusLine, setSpinnerActive, makeFallbackGitState } from "./statusline";
import { installAutocompleteHintStyling, QolCompactPromptEditor, currentEditorText, getQolArgumentCompletions } from "./editor";
import { autoRenameEnabled, firstUserMessageText, generateAutoRenameName, withAutoRenamePrefix, conversationTranscriptText } from "./session-rename";
import { buildQolContextUsageDetails, renderQolContextUsageMessage } from "./context-usage";
import { sendQolNotification, notifyQuestionOpened } from "./notifications";
import { permissionGateMatch, permissionGatePrompt, permissionGateCommands } from "./permission-gate";
import { installThinkingTimerPatch, getThinkingTimerStore, thinkingTimerLabel, hiddenThinkingLabel } from "./thinking-timer";
import { resolveSubmittedImagePaths, attachmentLabels, imageContentForPath } from "./images";
...
export default function (pi: ExtensionAPI) {
    ...
    handler: async (args: string, ctx: ExtensionCommandContext) => {
      ...
      if (trimmed === "status" || !trimmed) {
        ...
        const lines = [
          ...
          `Auto session rename: ${autoRenameEnabled(cwd) ? `enabled (${settingString("sessionAutoRename.model", "openai-codex/gpt-5.4-mini", cwd)})` : "disabled"}`,
      ...
      if (trimmed === "rename" || trimmed === "rename full") {
        ...
        const text = full ? conversationTranscriptText(branch, 2000) : firstUserMessageText(branch);
        if (!text) { ctx.ui.notify("No messages to rename from", "warning"); return; }
        ctx.ui.notify("Generating session name...", "info");
        const result = await generateAutoRenameName(text, ctx, full);
        if (result.name) {
          pi.setSessionName(withAutoRenamePrefix(result.name, ctx.cwd));
          ctx.ui.notify(`Session renamed: ${result.name} (${result.source})`, "info");
        } else {
      ...
      ctx.ui.notify("Usage: /qol [status|rename|rename full|notify-test]", "info");
    },
  });
  ...
  pi.registerCommand("context", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      if (!ctx.hasUI) { ctx.ui.notify("/context requires interactive mode", "error"); return; }
      if (!buildQolContextUsageDetails) return;
      const timer = setTimeout(() => ctx.ui.notify("Analyzing context...", "info"), 500);
      timer.unref?.();
      const details = buildQolContextUsageDetails(pi, ctx, ctx.getSystemPromptOptions?.());
      clearTimeout(timer);
      if (!details) { ctx.ui.notify("No context usage data available", "warning"); return; }
      pi.sendMessage({
  ...
  // Thinking timer: track thinking block start times via stream events, with live refresh
  {
    ...
    (globalThis as any)[Symbol.for("pi-qol.thinking-timer.store")] = timerStore;
    ...
    function finalizeThinkingBlock(key: string, endTimeMs = Date.now()) {
      ...
      if (label) label.setText(thinkingTimerLabel(timerStore.theme, duration, timerStore.cwd));
      if (timerStore.starts.size === 0) {
        if (thinkingTicker) { clearInterval(thinkingTicker); thinkingTicker = undefined; }
      }
    }
    ...
    function tickLabels() {
      ...
      for (const [key, start] of timerStore.starts) {
        const label = timerStore.labels.get(key);
        if (label) label.setText(thinkingTimerLabel(timerStore.theme, Date.now() - start, timerStore.cwd));
      }
    }
    ...
    pi.on("message_update", (event: any, ctx: any) => {
      ...
      if (streamEvent.type === "thinking_start" || streamEvent.type === "thinking_delta") {
        ...
        if (!timerStore.starts.has(key) && !timerStore.durations.has(key)) timerStore.starts.set(key, Date.now());
        startTicker();
        tickLabels();
        return;
      }
      if (streamEvent.type === "thinking_end") {
        const partial = streamEvent.partial;
        if (!partial || typeof partial.timestamp !== "number" || typeof streamEvent.contentIndex !== "number") return;
        finalizeThinkingBlock(`${partial.timestamp}:${streamEvent.contentIndex}`);
      }
    });
  ...
  pi.on("session_start", async (event: any, ctx: any) => {
    ...
    refreshGitState(pi, ctx).then((state) => { currentGitState = state; updateStatusline(ctx); }).catch(() => {});

    // Replace default footer with empty render to hide pi's built-in statusline
    ctx.ui.setFooter((tui: any, _theme: any, _footerData: any) => ({
      render: () => [],
      invalidate() {},
      dispose() {},
    }));
    ...
    if (settingBoolean("compactPrompt", true, ctx.cwd)) {
      const paddingLines = Math.max(0, Math.floor(settingNumber("editor.inputBottomPaddingLines", 0, ctx.cwd)));
      ctx.ui.setEditorComponent((tui: any, editorTheme: any, keybindings: any) =>
        new QolCompactPromptEditor(tui, editorTheme, keybindings, paddingLines, ctx),
      );
    }
    ...
    // Auto-rename (on first session load)
    if (event.reason !== "reload" && autoRenameEnabled(ctx.cwd)) {
      const branch = ctx.sessionManager.getBranch();
      const text = firstUserMessageText(branch);
      if (text && !pi.getSessionName()) {
        generateAutoRenameName(text, ctx).then((result) => {
          if (result.name) pi.setSessionName(withAutoRenamePrefix(result.name, ctx.cwd));
        }).catch(() => {});
      }
    }
  ...
  pi.on("tool_call", async (event: any, ctx: any) => {
    ...
    const header = event.input?.header || event.input?.question || "Input required";
  ...
  pi.on("agent_end", async (event: any, ctx: any) => {
    ...
    if (lastText) {
      const criticalMatch = lastText.match(/\b(critical|urgent|warning|blocked|cannot proceed|security|vulnerab|secret|credential|rate limit|context (overflow|full)|manual action required)\b/i);
      if (criticalMatch) {
        sendQolNotification(ctx, "critical", `Critical: ${criticalMatch[0]}`, "error");
        return;
      }
      const needsDir = /\?\s*$|\b(let me know|tell me|which (one|option)|choose|confirm|approve|should i|would you like|do you want|need your input|awaiting|next step)\b/i.test(lastText);
      if (needsDir) {
  ...
  pi.on("input", async (event: any, ctx: any) => {
    if (!settingBoolean("showImageChips", true, ctx.cwd)) return { action: "continue" };
    ...
    const newImages = paths.map((path) => imageContentForPath(path)).filter(Boolean);
